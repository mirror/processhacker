...
PKPH_REQUEST_HANDLER KphRequestHandlerTable[KPH_NUM_CTL_CODES];
...
NTSTATUS KphRead(PKPH_REQUEST Request)
{
    PVOID address;
    
    if (Request->Type == KrInfo)
    {
        Request->Info.Name = "Read";
        Request->Info.RequiredInputLength = 4;
        return STATUS_SUCCESS;
    }
    
    address = *(PVOID *)Request->Request.InputBuffer;
    
    __try
    {
        RtlCopyMemory(
            Request->Request.OutputBuffer,
            address,
            Request->Request.OutputLength;
            );
        Request->Request.ReturnLength = Request->Request.OutputLength;
    }
    __except (EXCEPTION_EXECUTE_HANDLER)
    {
        Request->Request.Status = STATUS_ACCESS_VIOLATION;
    }
}

VOID KphInitializeHandlerTable()
{
    
}

NTSTATUS KphRequest(ULONG ControlCode, PKPH_REQUEST Request)
{
    return KphRequestHandlerTable[KPH_CTL_CODE_TO_INDEX(ControlCode)](Request);
}
...
NTSTATUS KphDispatchDeviceControl(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
    ...
    ULONG retLength = 0;
    ULONG auxDataLength = sizeof(NTSTATUS);
    ...
    if (controlCode < KPH_MIN_CTL_CODE || controlCode > KPH_MAX_CTL_CODE)
    {
        status = STATUS_INVALID_DEVICE_REQUEST;
        goto IoControlEnd;
    }
    
    if (outLength < auxDataLength)
    {
    ...
    
    {
        ...
        /* Request basic info first */
        request.Type = KrInfo;
        request.Info.Name = NULL;
        request.Info.RequiredInputLength = -1;
        request.Info.RequiredOutputLength = -1;
        status = KphRequest(controlCode, &request);
        
        if (!NT_SUCCESS(status))
            goto IoControlEnd;
        
        if (request.Info.Name)
            dprintf("IoControl 0x%08x (%s)\n", controlCode, request.Info.Name);
        
        if (request.Info.RequiredInputLength != -1)
        {
            if (inLength < request.Info.RequiredInputLength)
            {
                status = STATUS_BUFFER_TOO_SMALL;
                goto IoControlEnd;
            }
        ...
        if (request.Info.RequiredOutputLength != -1)
        {
            if (outLength < request.Info.RequiredOutputLength + auxDataLength)
            {
                status = STATUS_BUFFER_TOO_SMALL;
                goto IoControlEnd;
            }
        ...
        request.Type = KrRequest;
        request.Request.ControlCode = controlCode;
        request.Request.Status = STATUS_SUCCESS;
        request.Request.InputLength = inLength;
        request.Request.OutputLength = outLength;
        request.Request.ReturnLength = 0;
        request.Request.InputBuffer = dataBuffer;
        request.Request.OutputBuffer = dataBuffer + auxDataLength;
        status = KphRequest(controlCode, &request);
        retLength = request.Request.ReturnLength + auxDataLength;
        
        /* Fill in aux data */
        *(PNTSTATUS)dataBuffer = request.Request.Status;
        
        dprintf("IOCTL 0x%08x result was 0x%08x\n", controlCode, request.Request.Status);
    }
